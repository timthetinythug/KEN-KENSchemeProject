...
;; (cages puz) produces list of all cages
(define (cages puz)
  (map (lambda (x) (first x))
       (puzzle-constraints puz)))

;; (big-list puz) produces list of lists containing all cells with either
;;    make-guess or make-posn
(define (big-list puz)
  (map (lambda (x)
         (small-lists x (puzzle-board puz)))
       (cages puz)))

;; (small-lists sym puz) wrapper for pls-save-me
(define (small-lists sym puz)
  (pls-save-me sym puz 0 0 empty))

;; (pls-save-me sym puz counter-x counter-y lofp) produces list of positions
;;    containing cell position or guess 
(define (pls-save-me sym puz counter-x counter-y lofp)
  (cond
    [(empty? puz) lofp]
    [else (pls-save-me2 sym (first puz) puz counter-x counter-y lofp)]))

;; (pls-save-me2 sym puz counter-x counter-y lofp) produces list of positions
;;    containing cell position or guess based on data type
(define (pls-save-me2 sym row puz counter-x counter-y lofp)
  (cond
    [(empty? row) (pls-save-me sym (rest puz) 0 (add1 counter-y) lofp)]
    [(number? (first row)) (pls-save-me2 sym (rest row) puz (add1 counter-x) counter-y lofp)]
    [(guess? (first row))
     (cond [(symbol=? (guess-symbol (first row)) sym)
            (pls-save-me2 sym (rest row) puz (add1 counter-x) counter-y
                          (append lofp (cons (first row) empty)))]
           [else (pls-save-me2 sym (rest row) puz (add1 counter-x) counter-y lofp)])]
    [(symbol? (first row))
     (cond [(symbol=? (first row) sym)
            (pls-save-me2 sym (rest row) puz (add1 counter-x) counter-y
                          (append lofp (cons (make-posn counter-x counter-y) empty)))]
           [else (pls-save-me2 sym (rest row) puz (add1 counter-x) counter-y lofp)])]))

;; (wrap-big puz) wrapper for bigger-list
(define (wrap-big puz)
  (bigger-list (cages puz) (big-list puz)))

;; (bigger-list lsit1 list2) adds the cage symbol for specific cells 
(define (bigger-list list1 list2)
  (cond
    [(empty? list1) empty]
    [else (cons (cons (first list1) (first list2))
                (bigger-list (rest list1) (rest list2)))]))

;; Examples 
(check-expect (wrap-big puzzle1)
              (list
               (list 'a (make-posn 0 0) (make-posn 0 1))
               (list 'b (make-posn 1 0) (make-posn 2 0))
               (list 'c (make-posn 3 0))
               (list 'd (make-posn 1 1) (make-posn 1 2))
               (list 'e (make-posn 2 1) (make-posn 3 1))
               (list 'f (make-posn 0 2) (make-posn 0 3))
               (list 'g (make-posn 2 2) (make-posn 3 2))
               (list 'h (make-posn 1 3))
               (list 'i (make-posn 2 3) (make-posn 3 3))))
(check-expect (big-list puzzle1)
              (list
               (list (make-posn 0 0) (make-posn 0 1))
               (list (make-posn 1 0) (make-posn 2 0))
               (list (make-posn 3 0))
               (list (make-posn 1 1) (make-posn 1 2))
               (list (make-posn 2 1) (make-posn 3 1))
               (list (make-posn 0 2) (make-posn 0 3))
               (list (make-posn 2 2) (make-posn 3 2))
               (list (make-posn 1 3))
               (list (make-posn 2 3) (make-posn 3 3))))
...
(define (find-blank puz)
  (cond
    [(equal? (foldr (lambda (x y) (+ x y)) 0
                    (map (lambda (x)
                           (length x)) (wrap-big puz)))
             (length (puzzle-constraints puz))) false]
    [else (find-blank2 (first (big-list puz)))]))

(define (find-blank2 lofp)
  (cond
    [(empty? lofp) 'guess]
    [(posn? (first lofp)) (first lofp)]
    [else (find-blank2 (rest lofp))]))
...
(define (used-in-row puz pos)
  (local [(define vertical (posn-y pos))]
    (quicksort (filter number? (map (lambda (x)
                                      (cond [(guess? x) (guess-number x)]
                                            [(number? x) x]
                                            [else null]))
                                    (list-ref (puzzle-board puz) vertical))) <)))
...
(check-expect (used-in-column puzzle1partial (make-posn 1 3)) (list 2 3))
(check-expect (used-in-column puzzle1partial (make-posn 1 0)) (list 2 3))
...
(define (available-vals puz pos)
  (foldr (lambda (x y)
           (cond [(not (member? x (append (used-in-row puz pos) (used-in-column puz pos))))
                  (cons x y)]
                 [else y])) empty (allvals (puzzle-size puz)))) 
...
(define (place-guess brd pos val)
  (place-guess2 brd pos val 0))

(define (place-guess2 brd pos val counter-y)
  (cond
    [(empty? brd) empty]
    [(equal? counter-y (posn-y pos))
     (cons (place-guess3 (first brd) pos val 0) (place-guess2 (rest brd) pos val (add1 counter-y)))]
    [else (cons (first brd) (place-guess2 (rest brd) pos val (add1 counter-y)))]))

(define (place-guess3 row pos val counter-x)
  (cond
    [(empty? row) empty]
    [(number? (first row)) (cons (first row) (place-guess3 (rest row) pos val (add1 counter-x)))]
    [(guess? (first row)) (cons (first row) (place-guess3 (rest row) pos val (add1 counter-x)))]
    [(equal? counter-x (posn-x pos)) (cons (make-guess (first row) val) (place-guess3 (rest row) pos val (add1 counter-x)))]
    [else (cons (first row) (place-guess3 (rest row) pos val (add1 counter-x)))]))
...
(define (guess-valid? puz)
  (local [(define lofn (quicksort (map guess-number (filter guess? (foldl append empty (big-list puz)))) >))
          (define operator (third (first (puzzle-constraints puz))))
          (define numb (second (first (puzzle-constraints puz))))]
    (cond [(symbol=? '+ operator) (= numb (foldr + 0 lofn))]
          [(symbol=? '* operator) (= numb (foldr * 1 lofn))]
          [(symbol=? '= operator) (= numb (first lofn))]
          [(symbol=? '- operator) (= numb (- (first lofn)(second lofn)))]
          [(symbol=? '/ operator) (= numb (/ (first lofn) (second lofn)))])))
...
(define (apply-guess puz)
  (local [(define sym (first (first (puzzle-constraints puz))))]
    (make-puzzle (puzzle-size puz)
                 (map (lambda (x)
                        (map (lambda (val)
                               (cond [(and (guess? val)
                                           (symbol=? sym (guess-symbol val)))
                                      (guess-number val)]
                                     [else val]))
                             x)) (puzzle-board puz))
...
(define (neighbours puz)
  (local [(define blank (find-blank puz))]
    (cond [(boolean? blank) empty]
          [(and (symbol? blank)
                (guess-valid? puz))
           (cons (apply-guess puz) empty)]
          [(and (symbol? blank)
                (not (guess-valid? puz)))
           empty]
          [else (map (lambda (x) (fill-in-guess puz blank x))
                     (available-vals puz blank))])))
...
;; the important one.
(check-expect (time (solve-kenken puzzle1 'fast)) puzzle1soln)
(check-expect (time (solve-kenken puzzle2 'fast)) puzzle2soln)
